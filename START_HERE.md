...
2. **Tự đặt sản phẩm:** tìm/lọc trong danh sách phân trang, **chọn rõ một món, chỉnh kích thước hiển thị rồi kéo thẻ sản phẩm vào ảnh**; bấm ảnh không được tự đặt món. Có thể đặt nhiều món, rồi chỉnh vị trí, góc xoay, lật và thứ tự lớp. Trên điện thoại dùng cùng luồng chạm giữ, kéo và thả. Căn phối cảnh là **tùy chọn**: nếu dùng, chấm bốn đỉnh của một **ô chữ nhật thực trên vùng sàn nhìn thấy** theo thứ tự **trước trái → sau trái → sau phải → trước phải**. Không chấm toàn bộ chu vi phòng, không đoán góc sau phần nhà vệ sinh lồi, không chấm tường/trần. Có thể dùng ron gạch, mép thảm hoặc ván sàn hình chữ nhật; nếu không thấy mốc thật rõ thì bỏ qua. [cameraSolver.js](client/src/utils/cameraSolver.js) chỉ dùng bốn điểm để lấy hai hướng phối cảnh, không tái dựng hình dạng toàn phòng.
...
- Dataset hiện tại có **69 sản phẩm**, 69 URL Shopee và 69 ID/slug riêng; toàn bộ `price` vẫn là `0`. Có 57 ảnh PNG local và 12 sản phẩm đang chờ Admin thêm ảnh. Giao diện phải hiện placeholder rõ ràng, đưa món có ảnh lên trước và không cho gửi món thiếu ảnh tham chiếu sang AI.
