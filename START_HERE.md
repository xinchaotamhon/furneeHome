...
4. **Xem và lưu:** bản ghép Canvas xuất hiện trước, sau đó gửi ảnh gốc, guide toàn scene, mặt nạ alpha sát từng vật và sheet tham chiếu sản phẩm đến AI (`mode: placement`). Kết quả AI chỉ được ghép qua vùng mặt nạ; lớp ảnh sản phẩm trong suốt được phủ lại để không mất hoặc đổi hình dạng món đã đặt. Spinner, nút `Ảnh gốc`/`Ảnh AI` và kết quả nằm ngay trong khung phòng. AI lỗi không được xóa scene hay kết quả trước. Nút Gợi ý AI tạo một kết quả `inspiration` riêng và không sửa danh sách món người dùng đang đặt thủ công.
