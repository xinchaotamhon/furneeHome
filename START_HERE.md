...
3. **Khai báo điều AI cần giữ:** ngay dưới danh sách món đã chọn chỉ có ba trường `Vị trí bạn muốn`, `Bạn không muốn`, `Ghi chú khác`; dữ liệu từng món gồm `usageType`, `placementSurface`, kích thước cm nếu nguồn ghi rõ đơn vị và `aiDescription` được lấy hoặc suy ra từ dữ liệu sản phẩm. Không bịa kích thước khi thiếu dữ liệu. Thứ tự ưu tiên tạo ảnh là **kiến trúc phòng và đúng sản phẩm/công năng → vị trí, tỷ lệ, layer → phong cách**. Ví dụ bàn ngồi bệt phải giữ chân ngắn, mặt bàn thấp và không tự thêm ghế cao.
4. **Xem và lưu:** bản ghép Canvas xuất hiện trước, sau đó gửi ảnh gốc, guide toàn scene, mặt nạ alpha sát từng vật và ảnh tham chiếu sản phẩm đến AI (`mode: placement`). Guide gửi provider có thể được thu nhỏ theo giới hạn model, nhưng mặt nạ ghép kết quả và lớp giữ chi tiết phải dùng kích thước ảnh đầu ra. Kết quả AI chỉ được ghép qua vùng mặt nạ có thêm viền hòa sáng/bóng nhỏ; lớp sản phẩm gốc chỉ phủ nhẹ để giữ nhận dạng, không được che phần ánh sáng và chất liệu AI vừa tạo. Preview trên trang và Canvas gửi AI phải dùng cùng một phép biến đổi để món không nhảy hình dáng sau khi tạo. Spinner, nút `Ảnh gốc`/`Ảnh AI` và kết quả nằm ngay trong khung phòng. AI lỗi không được xóa scene hay kết quả trước. Nút Gợi ý AI tạo một kết quả `inspiration` riêng và không sửa danh sách món người dùng đang đặt thủ công.
...
Collection lưu `designMode` (`placement`/`inspiration`), `resultImage`, ảnh gốc, `placements` cùng product facts, `scaleReference { points, lengthCm }`, `markedCorners` legacy, `designBrief`, `userPrompt`, model và thời gian tạo. Tọa độ lưu chuẩn hóa 0–1. Ý tưởng AI có thể có `placements: []`; không tạo sản phẩm giả. Mẫu công khai đọc từ MongoDB nên dùng chung giữa người dùng, có tác giả, lượt tim và quan hệ mẫu gốc/biến thể. Danh sách chỉ tải `previewImage` 560 px; ảnh đầy đủ chỉ tải khi mở một mẫu. Chỉ người đăng nhập mới được công khai, thả tim, lưu hoặc tạo biến thể; dùng lại luôn tạo một bản riêng tư mới.
...
- Modal [LoginModal.jsx](client/src/components/auth/LoginModal.jsx) đăng ký theo ba bước: gửi email, xác minh OTP 6 số rồi mới cho tạo tên và mật khẩu. Đăng nhập dùng email và mật khẩu.
- Backend [authController.js](server/src/controllers/authController.js) chỉ lưu hash OTP, giới hạn gửi theo email và địa chỉ mạng đã băm, giới hạn nhập sai, hash mật khẩu bằng bcryptjs và trả JWT 7 ngày sau khi hoàn tất. Production phải có SMTP; `AUTH_OTP_DEV_MODE=true` chỉ trả OTP cho request localhost khi không chạy production.
...
- JSON dự phòng và MongoDB có thể có số lượng khác nhau trong lúc nhóm đang bổ sung sản phẩm; MongoDB là nguồn dùng chung, JSON cùng ảnh local là phần cần commit để các máy có cùng fallback. API kiểm tra đường dẫn ảnh local có tồn tại thật, đưa món thiếu ảnh xuống sau, hiện trạng thái `Cần ảnh` ở Admin và không cho gửi món thiếu ảnh tham chiếu sang AI.
...
- Admin dùng [AdminPage.jsx](client/src/pages/AdminPage.jsx) để thêm, xem, sửa, xóa sản phẩm qua API, thêm tối đa 6 ảnh và tải JSON mới nhất. Import URL Shopee là khối riêng chỉ hiện trên localhost để có thể bỏ khỏi bản bảo vệ mà không ảnh hưởng CRUD. Import local đọc shop ID/item ID, lưu MongoDB và đồng bộ JSON. Với URL `...i.<shopId>.<itemId>`, ảnh local tự mang tên item ID theo định dạng đã tải: PNG `/images/products/<itemId>.png` (ưu tiên ảnh tách nền trong suốt), JPG `/images/products/<itemId>.jpg`, WebP `/images/products/<itemId>.webp`; cần commit ảnh và JSON khi cập nhật repo.
...
Tham khảo UX đã chọn từ CLOVER: [React Bits](https://github.com/DavidHDev/react-bits) cho hiệu ứng nhẹ, [Open Design](https://github.com/nexu-io/open-design) cho cách tổ chức vùng làm việc và [React Flow](https://reactflow.dev/examples/layout/dagre) cho ý tưởng sơ đồ. Chỉ lấy ý tưởng, không thêm các repo/framework này làm dependency. `fourgether/` là repo luồng học riêng theo Input → Process → Output, dùng font hệ thống hỗ trợ tiếng Việt; tiến độ chỉ giữ trong phiên, không lưu bộ nhớ trình duyệt. Khi sửa luồng học, chạy `node --test smoke.cjs` ngay trong thư mục đó.
...
- Đăng nhập, đăng ký xác minh email và Admin CRUD dùng API thật, không dùng dữ liệu demo phía frontend.
