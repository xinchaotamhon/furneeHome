...
export default function PublicCollectionDetailPage() {
  ...
  const useTemplate = async () => {
    ...
    setMessage('');
    if (!user) {
      setMessage('Đăng nhập để dùng mẫu này.');
      openLogin('login');
      return;
    }
    try {
      const reused = await roomDesignService.reuse(design._id);
      const privateFork = addRemoteRoomTemplate(reused || design) || reused || design;
      saveHandoff(privateFork);
      navigate('/room-studio');
    } catch {
      setMessage('Chưa thể tạo biến thể. Hãy thử lại.');
    }
  };
  ...
  return <main className="container page">
    <div className="page-heading"><p className="eyebrow">MẪU CÔNG KHAI</p><h1>{design.name}</h1>{design.creator?.id && <Link className="public-creator detail" to={`/collections/public/creator/${design.creator.id}`}>{design.creator.name || 'Thành viên FurneeHome'}</Link>}</div>
    ...
    <section className="saved-card"><div><span className="category-label">SẢN PHẨM</span><h2>{design.productName || 'Danh sách sản phẩm'}</h2>{products.length > 0 && <ul>{products.map((product, index) => <li key={product._id || product.id || index}>{product.name || product.productName || product.title || 'Sản phẩm nội thất'}</li>)}</ul>}</div><div className="saved-actions"><button className="button" type="button" onClick={useTemplate}>{user ? 'Tạo biến thể' : 'Đăng nhập để dùng mẫu'}</button>{!user && <button className="text-button" type="button" onClick={openRegister}>Tạo tài khoản</button>}<Link className="text-button" to="/collections/public">← Xem mẫu khác</Link></div></section>
