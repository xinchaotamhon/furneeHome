...
export default function ProductListPage() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = JSON.parse(sessionStorage.getItem(ROOM_STUDIO_SESSION_KEY)) || {};
      sessionStorage.setItem(ROOM_STUDIO_SESSION_KEY, JSON.stringify({
        selectedIds: selectedForRoom,
        desiredPositions: saved.desiredPositions && typeof saved.desiredPositions === 'object' ? saved.desiredPositions : {},
      }));
    } catch {}
  ...
  return (
    <main className="container page">
      <div className="page-heading split-heading">
        ...
        <div className="result-count"><strong>{total}</strong><span>sản phẩm phù hợp</span></div>
      </div>
      ...
      {loading && !products.length ? (
        ...
        <>
          <ProductGrid products={products} roomSelection={roomSelection} selectedIds={selectedForRoom} onToggleRoomProduct={toggleRoomProduct} />
          {pageCount > 1 && (
