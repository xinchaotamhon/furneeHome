...
export default function AdminPage() {
  ...
  const visibleProducts = useMemo(() => {
    ...
    return products.filter((product) => product.isActive !== false && (!search || `${product.name} ${categoryName(product)}`.toLocaleLowerCase('vi').includes(search)));
  }, [products, query]);
  ...
  async function deleteProduct(product) {
    if (!window.confirm(`Ngừng bán sản phẩm “${product.name}”?`)) return;
    setWorking(true); setNotice(''); setError('');
    try { await removeProduct(product._id || product.id); setNotice('Đã ngừng bán sản phẩm.'); if (editingId === (product._id || product.id)) resetForm(); } catch (deleteError) { setError(messageFrom(deleteError)); } finally { setWorking(false); }
  }
  ...
  async function updateOrder(id, orderStatus) {
    setWorking(true); setNotice(''); setError('');
    try { const saved = await orderService.updateOrderStatus(id, orderStatus); setOrders((current) => current.map((order) => (order._id === id ? saved : order))); setNotice('Đã cập nhật trạng thái đơn hàng.'); } catch (updateError) { setError(messageFrom(updateError)); } finally { setWorking(false); }
  }
  ...
  return <main className="container page admin-page">
    ...
    {tab === 'products' && <div className="admin-layout">
      ...
      <section className="admin-products panel-card"><div className="section-title"><h2>Danh sách sản phẩm ({visibleProducts.length})</h2><button className="text-button" type="button" onClick={refreshProducts} disabled={loading || isWorking}>{loading ? 'Đang tải…' : 'Tải lại'}</button></div><input className="admin-search" type="search" value={query} placeholder="Tìm tên hoặc danh mục" onChange={(event) => setQuery(event.target.value)} /><div className="admin-product-list">{visibleProducts.map((product) => { const id = product._id || product.id; return <article key={id}><div className="admin-thumb"><ProductArtwork product={product} /></div><div className="admin-product-name"><strong>{product.name}</strong><span>{categoryName(product)} · {formatPrice(product.price)} · Còn {product.stock ?? 0}</span></div><div className="row-actions"><button className="admin-edit" type="button" onClick={() => startEditing(product)}>Sửa</button><label className="admin-upload">{uploadingId === id ? 'Đang lưu…' : 'Thêm ảnh'}<input hidden type="file" accept="image/png,image/jpeg,image/webp" onChange={(event) => uploadImage(product, event.target.files?.[0])} /></label><button className="admin-delete" type="button" onClick={() => deleteProduct(product)}>Ngừng bán</button></div></article>; })}</div>{!visibleProducts.length && <p className="muted">Không tìm thấy sản phẩm.</p>}</section>
    </div>}

    {tab === 'customers' && <section className="panel-card admin-table-card"><div className="section-title"><h2>Khách hàng ({customers.length})</h2><button className="text-button" type="button" onClick={loadUsers}>Tải lại</button></div>{isWorking && !customers.length ? <p className="muted">Đang tải…</p> : <div className="admin-user-list">{customers.map((account) => { const id = account._id || account.id; return <article key={id}><div><strong>{account.name}</strong><span>{account.email}</span></div><span className="admin-role">Khách hàng</span><button type="button" className={account.isActive ? 'admin-delete' : 'admin-edit'} onClick={() => updateAccount(id, { isActive: !account.isActive })} disabled={isWorking}>{account.isActive ? 'Khóa' : 'Mở khóa'}</button></article>; })}</div>}</section>}

    {tab === 'orders' && <section className="panel-card admin-table-card"><div className="section-title"><h2>Đơn hàng ({orders.length})</h2><button className="text-button" type="button" onClick={loadOrders}>Tải lại</button></div>{isWorking && !orders.length ? <p className="muted">Đang tải…</p> : !orders.length ? <p className="muted">Chưa có đơn hàng.</p> : <div className="admin-table-container"><table className="admin-table"><thead><tr><th>Mã đơn</th><th>Khách hàng</th><th>Sản phẩm</th><th>Tổng</th><th>Trạng thái</th></tr></thead><tbody>{orders.map((order) => <tr key={order._id}><td>{String(order._id).slice(-8).toUpperCase()}</td><td>{order.shippingAddress?.fullName}<br /><small>{order.shippingAddress?.phone}</small></td><td>{(order.orderItems || []).map((item) => `${item.name} × ${item.qty ?? item.quantity}`).join(', ')}</td><td>{formatPrice(order.totalAmount)}</td><td><select value={order.orderStatus || 'Pending'} disabled={isWorking} onChange={(event) => updateOrder(order._id, event.target.value)}>{ORDER_STATES.map((state) => <option key={state}>{state}</option>)}</select></td></tr>)}</tbody></table></div>}</section>}
    ...
    {isSuperadmin && tab === 'admins' && <section className="panel-card admin-table-card"><div className="section-title"><h2>Quản trị admin</h2><button className="text-button" type="button" onClick={loadUsers}>Tải lại</button></div>{isWorking && !managedAccounts.length ? <p className="muted">Đang tải…</p> : <div className="admin-user-list">{managedAccounts.map((account) => { const id = account._id || account.id; return <article key={id}><div><strong>{account.name}</strong><span>{account.email}</span></div><select value={account.role} onChange={(event) => updateAccount(id, { role: event.target.value })} disabled={isWorking}><option value="customer">Khách hàng</option><option value="admin">Admin</option></select><button type="button" className={account.isActive ? 'admin-delete' : 'admin-edit'} onClick={() => updateAccount(id, { isActive: !account.isActive })} disabled={isWorking}>{account.isActive ? 'Khóa' : 'Mở khóa'}</button></article>; })}</div>}</section>}
  </main>;
}
