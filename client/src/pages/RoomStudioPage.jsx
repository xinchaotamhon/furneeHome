...
export default function RoomStudioPage() {
  ...
  return <main className="container page simple-studio"><div className="page-heading"><p className="eyebrow">ROOM STUDIO</p><h1>Thử một món trong phòng của bạn</h1><p>Không có công cụ kéo, căn góc hay chỉnh phối cảnh: chỉ ảnh phòng, một sản phẩm và ảnh AI.</p></div><div className="studio-simple-steps"><section className="panel-card"><span className="step-label">BƯỚC 1</span><h2>Tải ảnh phòng</h2><label className="simple-upload"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={upload} />{roomImage ? <img src={roomImage} alt="Phòng của bạn" /> : <strong>Chọn ảnh phòng</strong>}</label></section><section className="panel-card"><span className="step-label">BƯỚC 2</span><h2>Chọn một sản phẩm</h2>{loading ? <p>Đang tải sản phẩm…</p> : <select value={selectedId} onChange={(event) => { setSelectedId(event.target.value); resetResult(roomImage, event.target.value); }}><option value="">Chọn sản phẩm</option>{products.map((product) => <option key={idOf(product)} value={idOf(product)}>{product.name}</option>)}</select>}{selected && <div className="simple-selected-product"><ProductArtwork product={selected} /><strong>{selected.name}</strong></div>}</section><section className="panel-card"><span className="step-label">BƯỚC 3</span><h2>Tạo và lưu</h2><button className="button" type="button" disabled={isGenerating || !roomImage || !selected} onClick={generate}>{isGenerating ? 'Đang tạo ảnh…' : 'Generate image'}</button><button className="button button-secondary" type="button" disabled={!resultImage} onClick={() => setShowResult((value) => !value)}>{showResult ? 'Xem ảnh gốc' : 'So sánh kết quả'}</button><button className="text-button" type="button" disabled={!resultImage} onClick={save}>Lưu vào Bộ sưu tập</button></section></div><p className="studio-message" role="status">{message}</p>{roomImage && <section className="simple-room-compare"><figure><figcaption>{showResult && resultImage ? 'Ảnh tạo / xem trước' : 'Ảnh gốc'}</figcaption><img src={showResult && resultImage ? resultImage : roomImage} alt="Kết quả thử sản phẩm" /></figure>{resultImage && <figure><figcaption>Ảnh gốc</figcaption><img src={roomImage} alt="Ảnh phòng gốc" /></figure>}</section>}</main>;
}
