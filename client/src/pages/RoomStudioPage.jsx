  ...
  return <main className="container page room-studio-page simple-room-studio">
    ...
    <div className="room-studio-layout">
      ...
      <aside className="room-controls-panel">
        ...
        <section className="room-control-section"><div className="room-section-title"><span>3</span><div><h2>Chỉnh món đang chọn</h2><p>{activePlacement ? activePlacement.productName : 'Chọn một món trên ảnh để chỉnh.'}</p></div></div>{activePlacement ? <div className="placement-tools"><div className="placement-tool-row"><label>Kích thước</label><button type="button" onClick={() => updatePlacement(activePlacement.id, { scale: Number(clamp(activePlacement.scale - 0.1, 0.4, 1.8).toFixed(1)) })}>−</button><span>{Math.round(activePlacement.scale * 100)}%</span><button type="button" onClick={() => updatePlacement(activePlacement.id, { scale: Number(clamp(activePlacement.scale + 0.1, 0.4, 1.8).toFixed(1)) })}>+</button></div><div className="placement-tool-row"><label>Xoay</label><button type="button" onClick={() => updatePlacement(activePlacement.id, { rotation: normalizeRotation(activePlacement.rotation - 15) })}>↺</button><span>{normalizeRotation(activePlacement.rotation)}°</span><button type="button" onClick={() => updatePlacement(activePlacement.id, { rotation: normalizeRotation(activePlacement.rotation + 15) })}>↻</button></div><div className="placement-action-row"><button type="button" className="button button-small button-secondary" onClick={() => updatePlacement(activePlacement.id, { isFlipped: !activePlacement.isFlipped })}>Lật ngang</button><button type="button" className="button button-small button-danger" onClick={() => removePlacement(activePlacement.id)}>Xóa món</button></div><button type="button" className="button button-small" onClick={() => void renderScene(placementsRef.current, activePlacement.id)}>{lastFailedPlacementId === activePlacement.id ? 'Thử lại ảnh AI' : 'Cập nhật ảnh AI cho cả phòng'}</button></div> : <p className="placement-empty">Sản phẩm đã đặt sẽ hiện ở đây để bạn chỉnh.</p>}<div className="room-save-actions"><button type="button" className="button" disabled={!placements.length} onClick={saveCollection}>Lưu vào Bộ sưu tập</button><small>Vị trí, kích thước và hướng của các món đều được lưu.</small></div></section>
      </aside>
    </div>
